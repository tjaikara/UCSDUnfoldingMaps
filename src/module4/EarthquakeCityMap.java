...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {
		...
		fill(color(255, 0, 0));
		triangle(55f, 105f - (traingleHeight-triangleInnerRadius), 55f - (traingleSideLength/2.0f),
				105f + triangleInnerRadius, 55f + (traingleSideLength/2.0f), 105f + triangleInnerRadius);
	...
	{
		String oceanQueaks = "";
		System.out.println("\nNumber of EarthQuake Occurred:");
		for(Marker  marker : countryMarkers){

			int numofQuakes=0;
			int numofOceanQuakes=0;

			for(Marker marker1 : quakeMarkers){

				if(marker1 instanceof LandQuakeMarker){

					if(((LandQuakeMarker) marker1).getCountry().equals(marker.getProperty("name"))){
						numofQuakes++;
					}
				}
				else if(numofOceanQuakes != -1){
					numofOceanQuakes++;
					oceanQueaks = "Ocean Quakes : " + numofOceanQuakes;
				}

			}
			numofOceanQuakes=-1;
			if( numofQuakes > 0)
				System.out.println(marker.getProperty("name") + " : " + numofQuakes);
		}
		System.out.println(oceanQueaks);
	}
